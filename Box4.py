...
def calculate_items_that_fit_in_box(box, item_volume, box_data):
    ...
    # Return the minimum of the two to ensure the number of items that fit both by volume and weight
...
def get_packing_info(items_and_quantities):
    ...
    if not items_data or not boxes_data:
        print("No data loaded for items or boxes.")
        return []
    ...
    for item_code, quantity in items_and_quantities:
        if item_code not in items_data:
            print(f"Error: Item code '{item_code}' not found in item data.")
            continue  # Skip this item if it's not found in the data
        ...
        for box_type, box in boxes_data.items():
            if check_box_fit(box, total_item_volume, total_weight):
                ...
                items_packed = min(items_that_fit, quantity)  
...
if not results:
    ...
    for result in results:
        if "message" in result:
            ...
            print(f"  Items actually packed in box: {result['items_packed']}")
            print()
